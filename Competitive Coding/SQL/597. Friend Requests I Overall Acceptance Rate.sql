...
round(
    coalesce(
        1.0*(select  count(distinct(requester_id, accepter_id)) from RequestAccepted as A)/
        nullif((select  count(distinct(sender_id,send_to_id)) from FriendRequest as B),0), 0 
    ),2
) as accept_rate


-- select round(coalesce(1.0*4/5,0),2)
