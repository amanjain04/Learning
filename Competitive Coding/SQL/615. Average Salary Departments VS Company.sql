...
with company_average as (
    select pay_date as pay_month
        , avg(amount) as c_amount
    from Salary s 
    group by pay_date
)
...
select distinct TO_CHAR(d.pay_month,'YYYY-MM') as pay_month
    , d.department_id
    , case when c.c_amount = d.d_amount and c.pay_month = d.pay_month then 'same'
     when c.c_amount > d.d_amount and c.pay_month = d.pay_month then 'lower'
     when c.c_amount < d.d_amount and c.pay_month = d.pay_month then 'higher'
     end as comparison
    from department_average d 
    inner join company_average c 
